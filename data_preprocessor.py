...
def construct_face_data(image_file, label_file):
    ...
    if num_lines != int(num_lines_images/70):
        print("Number of lines mismatch")
        return None

    image_matrix = np.zeros(shape=(num_lines,70,70))
    label_matrix = np.zeros(shape=num_lines)

    with open(image_file, "r") as open_image_file:
        line_handle = 0
        matrix_index = 0
        while line_handle < (num_lines_images - 69):
            hz_index = 0
            for line in itertools.islice(open_image_file, 0,  70):
                vt_index = 0
                for char in line:
                    if char == '#' or char == "+":
...
def construct_digit_data(image_file, label_file):
...
def digit_training_data(image_file="data/digitdata/trainingimages",
    ...
    return construct_digit_data(image_file, label_file)
...
def face_training_data(image_file="data/facedata/facedatatrain",
                            label_file="data/facedata/facedatatrainlabels"):
    return construct_face_data(image_file, label_file)


def face_test_data(image_file="data/facedata/facedatatest",
                            label_file="data/facedata/facedatatestlabels"):
    return construct_face_data(image_file, label_file)


def face_validation_data(image_file = "data/facedata/facedatavalidation",
                            label_file = "data/facedata/facedatavalidationlabels"):
    return construct_face_data(image_file, label_file)
